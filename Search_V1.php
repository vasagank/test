...
class Search extends Controller {  
    ...
    public function search($payload = []) {        
        ...
        if(!empty($query)) {            
            if($call_claude && !is_exact_match($sm_filter, $query)) {
                ...
                if(!empty($claude_obj)) {
                    ...
                    $term_relations = $claude_obj["term_relations"] ?? [];
                    ...
                    $excluded_terms = $topic_term_types["excluded"];
                    
                    
                    //echo "<pre>"; print_r($topic_term_types); die;
                    //echo "$query - $topic_key <pre>"; print_r($topic_term_types);die;
                    ...
                    if($query_type == "topic" || $query_type == "podcast_name") {
                        foreach($term_weights as $topic => $weight) {
                            ...
                            if(isset($alias_terms[$lookup_key])) {
                                $relation = "alias";
                            } elseif(isset($term_relations[$topic]) && strtolower(trim($term_relations[$topic])) == "alias") {
                                $relation = "alias";
                            } elseif(isset($excluded_terms[$lookup_key])) {
                                $relation = "excluded";
                            } elseif(isset($restatement_terms[$lookup_key])) {
                                $relation = "restatement";
                            } elseif(isset($broader_terms[$lookup_key])) {
                                $relation = "broader";
                            } elseif(isset($term_relations[$topic])) {
                                $relation = strtolower(trim($term_relations[$topic]));
                            }
                            ...
                            } elseif($relation == "broader") {
                                $boost_terms[] = $norm_topic;
                            } elseif($relation === null && $weight >= 0.9) {
                                $queries_arr[$norm_topic] = $weight;
                            }
                        }
                    }
                    
                    //echo "<pre>"; print_r($restatement_overrides);die;
        ...
        if(!empty($primary_term)):
            $vars["literal_query"] = $primary_term;
        endif;
        ...
        /*if(!empty($response_id)):
            ...
            if($this->email_id == "vasagan@feedspotmail.com"):
                $vars["debug"] = 0;
                /*$url = 'http://10.0.0.111:8080/solr/apple_podcast_index/select?q=*:*&rows=3&fl=id&sort=ms(NOW,last_original_article_creation_date) desc';
